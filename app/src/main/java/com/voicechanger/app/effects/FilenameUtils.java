...
public class FilenameUtils {
    ...
    public static String removeExtension(String strExtention) {
        if (strExtention == null) {
            return null;
        }
        failIfNullByte(strExtention);
        int indexOfExtension = extensionIndex(strExtention);
        if (indexOfExtension == -1) {
            return strExtention;
        }
        return strExtention.substring(0, indexOfExtension);
    }
